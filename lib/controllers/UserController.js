...
exports.getConfirmEmail = async function (req, res, next) {
    const notifications = [];
    let token = req.query.token;
    try {
        const exists = await User.userExists({ 'extras.emailConfirmationCode': token })
        if (!exists) throw new UserNotFound("This link is no longer valid.");
        const user = await User.getUser({ 'extras.emailConfirmationCode': token })
        await User.update({ email: user.email }, { 'extras.emailConfirmationCode': null, 'extras.emailConfirmed': true, });
...
exports.postCreateUser = async function (req, res, next) {
    let notifications = [];
    const user = {
    ...
    try {
        ...
        await sendConfirmationEmail(req, user, user.emailConfirmationCode, req.headers.host, error);
...
exports.getSendConfirmationEmail = async function (req, res, next) {
    try {
        ensureLoggedIn(req);
        const notifications = [];
        const error = {
        ...
        await sendConfirmationEmail(req, req.user, req.user.extras.emailConfirmationCode, req.headers.host, error)
