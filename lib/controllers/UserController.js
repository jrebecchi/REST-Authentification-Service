...
exports.updateUser = function (req, res, next) {
    ...
    if (userUpdates.email) {
        user.emailConfirmationCode = generateToken();
        const emailAnalysis = EmailValidator.analyse(req.body.email);
        notifications = notifications.concat(emailAnalysis.notifications);
        if ( req.body.old_password === undefined )
            notifications.push({ type: 'error', message: 'Please enter your previous password!' })
    }
    ...
    if (userUpdates.password && userUpdates.password !== req.body.old_password) {
        User.isPasswordValid({ email: req.user.email },  req.body.old_password)
            .then(isValid => {
                if (!isValid) {
                    throw new WrongPasswordError("Your previous password is wrong!");
                }
                return User.resetPassword({ email: req.user.email }, userUpdates.password);
            })
            .then(() => {
    ...
    if (userUpdates.username && userUpdates.username !== req.user.username) {
        User.userExists({ username: userUpdates.username })
            .then(usernameExists => {
                if (usernameExists)
                    throw new UsernameAlreadyExistsError("Username already exists");
                return User.update({ email: req.user.email }, { username: userUpdates.username });
            })
            .then(() => {
    ...
    if (userUpdates.email && userUpdates.email !== req.user.email) {
        User.userExists({ email: userUpdates.email })
            .then(emailExists => {
                if (emailExists)
                    throw new EmailAlreadyExistsError("Email already exists");
                emailConfirmationCode = generateToken();
                return User.update({ username: req.user.username }, { email: userUpdates.email, 'extras.emailConfirmationCode': emailConfirmationCode, "extras.emailConfirmed": false });
            })
            .then((user) => {
                notifications.push({ type: 'success', message: 'Your email is now updated !' })
                return sendConfirmationEmail(req, user, emailConfirmationCode, req.headers.host, error)
            })
            .then(() => {
                notifications.push({ type: 'info', message: 'You will receive a confirmation link at your email address in a few minutes.' })
                res.json({ notifications: notifications });
    ...
    if (userUpdates.extras) {
        User.update({ email: req.user.email }, { 'extras': userUpdates.extras })
        .then(() => {
