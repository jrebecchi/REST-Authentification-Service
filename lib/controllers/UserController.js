...
exports.sendConfirmationEmail = async function (req, res, next) {
    try {
        ...
        const userFullInfos = await User.getUser({_id: req.user.id})
        await sendConfirmationEmail(req, req.user, userFullInfos.emailConfirmationCode, req.headers.host, error)
...
exports.sendPasswordRecoveryEmail = function (req, res, next) {
    const notifications = [];
