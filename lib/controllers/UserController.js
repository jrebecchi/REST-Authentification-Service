...
exports.updateUser = async function (req, res, next) {
    ...
    for (let index in req.body) {
        if (index !== "username" && index !== "password" && index !== "confirmPassword" && index !== "email" && index !== "previous_password")
            userUpdates.extras[index] = req.body[index]
    }

    const updatePasswordToken = req.body["passwordRecoveryToken"]
    delete userUpdates["passwordRecoveryToken"];
    delete userUpdates.extras["passwordRecoveryToken"];
    ...
    if (userUpdates.password) {
        ...
        notifications = notifications.concat(passwordAnalysis.notifications);
        if (req.body.previous_password === undefined && updatePasswordToken === undefined)
    ...
    try {
        ...
        if (userUpdates.password) {
            ...
            } else if (userUpdates.password !== req.body.previous_password) {
