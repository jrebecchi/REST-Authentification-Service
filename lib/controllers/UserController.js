...
exports.updateUser = async function (req, res, next) {
    ...
    try {
        ...
        const user = await User.updateUser({ id: req.user.id }, userUpdates);
        notifications.push({ type: 'success', message: 'User information updated!' });
        if (userUpdates.email) await sendConfirmationEmail(req, user, emailConfirmationCode, req.headers.host, error)
        res.json({ notifications: notifications });
        const payload  = await User.sign({ id: req.user.id }, password, req.options.privateKey)
        res.json({
            ...payload,
            notifications
        })
