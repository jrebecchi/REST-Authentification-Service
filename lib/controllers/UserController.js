...
exports.createUser = async function (req, res, next) {
    ...
    try {
        ...
        if (err.message.includes("username")) err = new UsernameAlreadyExistsError("Username already exists");
        if (err.message.includes("email")) err = new EmailAlreadyExistsError("Email already exists");
        next(err)
    }
};
