...
const computeUserToken = (user) => {
    new Promise((resolve) => {
        jwt.sign(JSON.stringify(user), privateKey, { algorithm: 'RS256' }, (err, token) => {
            if (err) {
                throw new WrongTokenError(err.message);
            } else {
                await updateUser({ _id: user.id }, { token })
                resolve(token);
            }
        });
    });
...
User.statics.sign = async function (filter, password, privateKey) {
    ...
    const token = await computeUserToken(user);
    return { user, token };
};
...
User.statics.verify = function (token, publicKey) {
    return new Promise((resolve) => {
        jwt.verify(token, publicKey, { algorithm: 'RS256' }, (err, user) => {
            if (err) {
                throw new WrongTokenError('Please log in!')
            } else {
                resolve(user)
            }
        });
...
User.statics.updateUser = async function (filter, data) {
    if (data.password) {
        try {
            const results = await PasswordEncryption.hashAndSalt(password);
            data = {
                ...data,
                password: results.hash,
                passwordSalt: results.salt
