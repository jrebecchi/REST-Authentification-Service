...
const DEFAULT_CONFIRM_EMAIL_TEMPLATE = path.resolve(__dirname, './templates/emails/VerifyEmail.ejs');
const DEFAULT_RESET_PASSWORD_TEMPLATE = path.resolve(__dirname, './templates/email/ResetPassword.ejs')
const DEFAULT_VERIFY_EMAIL_TEMPLATE = path.resolve(__dirname, './templates/pages/VerifyEmail.ejs');
...
function createApplication(emailConfig, dbConfig, options) {
    ...
    if (options.verifyEmailTemplate === undefined) {
        options.verifyEmailTemplater = DEFAULT_VERIFY_EMAIL_TEMPLATE;
    } else {
    ...
    db.init(dbConfig);
    agenda.init(dbConfig);
    mailer.init(emailConfig);
    User.init({}, options.hasUsername);
    ...
    app.use(async (req, res, next) => {
        ...
        if (typeof bearerHeader !== 'undefined') {
            ...
            try {
                const user = await User().verify(bearerToken, req.options.publicKey);
                req.user = user;
                next();
            } catch (err) {
                next(err);
